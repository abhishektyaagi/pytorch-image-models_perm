...
from timm.loss import JsdCrossEntropy, SoftTargetCrossEntropy, BinaryCrossEntropy, LabelSmoothingCrossEntropy, l1_l2_penalty, threshold_and_normalize, auto_shuffle_penalty
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        penalty_lambda = 1e-3  # tune as appropriate

        def compute_autoshuffle_penalty(model):
            # Sum up the L1-L2 penalty for each AutoShuffleLinear
            penalty = torch.tensor(0., device=next(model.parameters()).device)
            for module in model.modules():
                if isinstance(module, AutoShuffleLinear):
                    # Suppose you have a function to compute: l1_l2_penalty(module.P_left) + l1_l2_penalty(module.P_right)
                    penalty += auto_shuffle_penalty(module)
            return penalty
        ...
        def _forward():
            ...
            #Compute L1-L2 penalty from all AutoShuffleLinear layers
            shuffle_pen = compute_autoshuffle_penalty(model)

            # Combine with normal loss
            loss = loss + penalty_lambda * shuffle_pen
        ...
        for module in model.modules():
            ...
            if isinstance(module, AutoShuffleLinear):
                module.apply_mask()
                module.post_update_clamp_and_norm()  # e.g. threshold & row/col normalize
        ############################################################################
